...
- Summary: Unreal Engine 5.8 Runtime 플러그인. PTZ 카메라 짐벌, Hucoms 호환 HTTP/MJPEG 서버, 시뮬레이터 HUD, `/scene/*` 런타임 씬 제어 API를 제공한다. 현재 버전 **0.1.9**(`.uplugin` `VersionName` 이 단일 출처 — 여기 적을 땐 그 파일을 확인하고 쓸 것).
